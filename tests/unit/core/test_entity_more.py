...
def test_collection_with_explicit_config_and_callable_filters():
    config = DummyConfig()
    items = [
        DummyWebElement('one', with_defaults=False),
        DummyWebElement('two', with_defaults=False),
...
def test_collection_element_by_its_accepts_callable_selector():
    ...
    found = collection.element_by_its(
        lambda row: row.element('.title'),
        title_has_text,
    )
...
def test_collection_should_reports_each_element_failures_after_fallback():
    ...
    assert 'indexes from 0 to 1' in str(error.value)
    assert 'no o' in str(error.value)
