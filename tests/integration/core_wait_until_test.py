...
def test_wait_until_for_not_present_returns_true_when_element_disappears(
    session_browser,
):
    ...
    matched = browser.element("#overlay").wait_until(be.not_.present)

    assert matched is True


def test_wait_until_for_not_present_returns_false_on_timeout(session_browser):
...
def test_wait_until_for_not_in_dom_returns_true_when_element_removed(session_browser):
    browser = session_browser.with_(timeout=0.5)
    page = GivenPage(browser.driver)
    page.opened_with_body(
        """
    ...
    matched = browser.element("#overlay").wait_until(be.not_.in_dom)

    assert matched is True


def test_wait_until_for_not_in_dom_returns_false_on_timeout(session_browser):
    browser = session_browser.with_(timeout=0.15)
    page = GivenPage(browser.driver)
    page.opened_with_body(
        """
...
def test_wait_until_for_present_returns_true_immediately_when_element_already_present(
    session_browser,
):
    browser = session_browser.with_(timeout=0.15)
    page = GivenPage(browser.driver)
    ...
    matched = browser.element("#ready").wait_until(be.present)
